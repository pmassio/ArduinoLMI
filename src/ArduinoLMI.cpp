...
Eigen::MatrixXf DecayRate( Eigen::MatrixXf A,  Eigen::MatrixXf B, float amin)  {
  ...
  int sizecon[NCON]={MATRIXN,MATRIXN}; // size of linear matrix inequality constraints
...
Eigen::MatrixXf DecayRateRobust1Param( Eigen::MatrixXf A0,  Eigen::MatrixXf A1, Eigen::MatrixXf B, float amin, float pmax, float pmin)  {
  ...
  int sizecon[NCON]={MATRIXN,MATRIXN,MATRIXN}; // size of linear matrix inequality constraints
  ...
  for(int i=sizeunk; i>=0;i--){
    ...
    V[1*(sizeunk+1)+i]->noalias()=-B*Y-Y.transpose()*B.transpose()-2*amin*X-X*A.transpose()-A*X; 
    A = A0+pmin*A1;
    V[2*(sizeunk+1)+i]->noalias()=-B*Y-Y.transpose()*B.transpose()-2*amin*X-X*A.transpose()-A*X; 
